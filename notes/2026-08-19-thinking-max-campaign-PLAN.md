...
Operator go-ahead: server default `thinking=max`, port recommended
items, A/B, then spark-eval, then public-safe repo sweep. Pause
no-think client for the exclusive window.
...
- no-think client MUST keep `thinking:false` (structured JSON in content).
- Public repo: no hostnames, IPs, usernames, tokens, private DNS.

## Sequence

1. Verify clients (no-think extractors / agent frontends / others).
2. Port patches into the head-node cluster checkout + git src:
...
7. Restore no-think client + canary + coverage audit.
8. Public-tree identifier sweep + push.

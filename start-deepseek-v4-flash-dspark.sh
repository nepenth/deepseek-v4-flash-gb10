...
if [ -n "${VLLM_SWITCH_KV_BLOCK_SIZE:-}" ]; then
  KV_BLOCK_SIZE="$VLLM_SWITCH_KV_BLOCK_SIZE"
fi
if ! [[ "$KV_BLOCK_SIZE" =~ ^[0-9]+$ ]]; then
  echo "KV_BLOCK_SIZE must be a positive integer (got: $KV_BLOCK_SIZE)" >&2
  exit 2
fi
if (( 10#$KV_BLOCK_SIZE < 1 )); then
  echo "KV_BLOCK_SIZE must be a positive integer (got: $KV_BLOCK_SIZE)" >&2
  exit 2
fi
...
scp "$SCRIPT_DIR/patches/hotfix-dsv4-issue31-v2-thinking-budget-gpu.py" "${WORKER_HOST}:${REMOTE_WORKER_DIR}/patches/hotfix-dsv4-issue31-v2-thinking-budget-gpu.py"
scp "$SCRIPT_DIR/patches/hotfix-dsv4-issue55-tool-truncation.py" "${WORKER_HOST}:${REMOTE_WORKER_DIR}/patches/hotfix-dsv4-issue55-tool-truncation.py"

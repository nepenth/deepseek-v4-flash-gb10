...
API_BASE="${VLLM_SWITCH_API_BASE:-http://127.0.0.1:8888}"
...
curl -fsS --max-time 10 "$API_BASE/v1/models" >"$OUT/models.json" 2>"$OUT/models.stderr" || true
curl -fsS --max-time 15 "$API_BASE/metrics" >"$OUT/metrics.txt" 2>"$OUT/metrics.stderr" || true

#!/usr/bin/env python3
"""Keep client stop strings dormant until </think> on vLLM 0.27.1 V2.

Port of Tony Patch 5 / Mia hotfix-dsv4-suppress-stops-in-reasoning.py.
Anemll 0.25 factory uses PreTrainedTokenizerFast; 0.27.1 uses TokenizersBackend.
Do not apply Mia's file unmodified.

Opt-out: DSPARK_SUPPRESS_STOPS_IN_REASONING=0 or VLLM_SUPPRESS_STOPS_IN_REASONING=0
"""
...
MARK = "# [suppress-stops-in-reasoning-v0271]"
...
FACTORY_NEW = '''        if USE_FAST_DETOKENIZER and isinstance(tokenizer, TokenizersBackend):
    ...
    def _maybe_enable_reasoning_stop_guard(detok, tokenizer, request) -> None:
        # [suppress-stops-in-reasoning-v0271] last prompt token == <think>
...
INIT_NEW = """        self._last_output_text_offset: int = 0
        ...
        # [suppress-stops-in-reasoning-v0271]
...
def main(argv: list[str]) -> int:
    ...
    if not target.is_file():
        print(f"[suppress-stops-v0271] missing {target}", file=sys.stderr)
        return 0
    text = target.read_text(encoding="utf-8")
    new, status = apply_text(text)
    if status == "applied":
        target.write_text(new, encoding="utf-8")
    print(f"[suppress-stops-v0271] {status}: {target}")

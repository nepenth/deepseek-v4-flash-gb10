...
Ported to vLLM 0.27.1 (`VLLMValidationError` V2 gate) and keep the
Whyland greedy `_requires_logits_processing` gate so temp=0 still fires.
Do not add an omit-field DEFAULT_THINKING_TOKEN_BUDGET.
...
from vllm.v1.worker.gpu.states import RequestState''',
    ...
    _patch_file(
        ...
        if np.any(self.logit_bias_state.use_logit_bias[idx_mapping_np]):
            return True''',
        f'''    def _requires_logits_processing(self, idx_mapping_np: np.ndarray) -> bool:
        if np.any(self.thinking_budget_state.use_budget[idx_mapping_np]):  # {MARK}
            return True
        if np.any(self.logit_bias_state.use_logit_bias[idx_mapping_np]):
            return True''',
        "greedy logits-processing gate",
    )
